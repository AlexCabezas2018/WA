...
class usersModel {
    constructor() {
        ...
        this.queries = {
            ...
            GET_CONVERSATION: "SELECT * FROM messages WHERE (user_from = ? AND user_to = ?) OR (user_from = ? AND user_to = ?) ORDER BY hora ASC",
            UPDATE_CONVERSATION: "INSERT INTO messages VALUES(?, ?, ?, ?, ?)",
        }
    }
    ...
    /**
     * Update the user puntuation given an email
     * @param {*} email 
     * @param {*} puntuation 
     * @param {*} callback 
     */
    ...
    getConversation(user_from, user_to, callback) {
        this.pool.getConnection((err, conn) => {
            if(err) callback(new Error(this.exceptions.connection_error), undefined);
            else {
                conn.query(this.queries.GET_CONVERSATION, [user_from, user_to, user_to, user_from], (err, result) => {
                    conn.release();
                    if(err) callback(new Error(err.message), undefined);
    ...
    //FIXME: Falla al insertar una fecha. En la base de datos aparece: 0000-00-00. Una solucion es que la bd lo inserte de manera automática, sin necesidad de pasárselo como parámetro.
    updateConversation(message, callback) {
        this.pool.getConnection((err, conn) => {
            if(err) callback(new Error(this.exceptions.connection_error), false);
            else {
                conn.query(this.queries.UPDATE_CONVERSATION, [null, message.user_from, message.user_to, message.msg, Date.now()],
                    (err, result) => {
                        conn.release();
