...
class usersModel {
    constructor() {
        ...
        this.queries = {
            ...
            GET_ALL_BY_EMAIL: 'SELECT * FROM users WHERE email = ?',
    ...
    updateAndGetUser(email, puntuation, callback) {
        this.updatePuntuation(email, puntuation, (err, result) => {
            if (err) callback(new Error(this.exceptions.connection_error), undefined);
            else {
                this.pool.getConnection((err, conn) => {
                    if (err) callback(new Error(this.exceptions.connection_error), undefined);
                    else {
                        conn.query(this.queries.GET_ALL_BY_EMAIL, [email],
                            (err, user) => {
                                conn.release();
                                if (err) callback(new Error(err.message), undefined);
                                else callback(undefined, user[0]);
                            });
                    }
                });
            }
